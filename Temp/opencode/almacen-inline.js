// 🔒 Control estricto de roles (Seguridad de Planta)
        ...
        function etiquetarTablas() {
            document.querySelectorAll('.tabla-card-movil table').forEach(tabla => {
                ...
                tabla.querySelectorAll('tbody tr').forEach(fila => {
                    if (/sep-categoria|sep-cat/.test(fila.className)) {
                        fila.cells.forEach(td => {
                            td.setAttribute('data-label', '');
                            td.classList.add('label-encabezado-grupo');
                        });
                        return;
                    }
                    ...
                    tbody.innerHTML += `
                        <tr>
                            ...
                            <td class="text-center">
                                ...
                                <button onclick="abrirModalAnularPendiente(${item.id})" class="btn-accion btn-anular" style="margin-left: 5px;">Anular</button>
                            </td>
                            </tr>
                        `;
                });
        ...
        let ingresoIntegracionActual = null;

        async function cargarInventarioSiVacío() {
        ...
        async function darConformidad(id) {
            ...
            await cargarInventarioSiVacío();

            const itemsPend = pendientesMap[id] || [];
            const nombresInv = new Set((inventarioGlobal || []).map(i => String(i.nombre || '').trim().toLowerCase()));

            const faltantes = itemsPend.filter(p =>
                p.nombre && !nombresInv.has(String(p.nombre).trim().toLowerCase())
            );

            if (faltantes.length > 0) {
                abrirModalIntegrarManualmente(id, faltantes);
                return;
            }
        ...
        // ⬇️ INTEGRACIÓN MANUAL DE ÍTEMS QUE NO EXISTEN EN EL INVENTARIO
        function abrirModalIntegrarManualmente(id, faltantes) {
            ingresoIntegracionActual = id;
            const datalist = document.getElementById('datalistInsumosNombres');
            if (datalist) datalist.innerHTML = (inventarioGlobal || []).map(i => `<option value="${i.nombre}"></option>`).join('');
            const tbody = document.getElementById('tablaIntegracionManual');
            tbody.innerHTML = '';
            faltantes.forEach(p => {
                const nomOrig = (p.nombre || '').replace(/"/g, '&quot;');
                const row = document.createElement('tr');
                row.innerHTML = `
                    <td>
                        <input type="text" class="integraNombre" list="datalistInsumosNombres" value="${nomOrig}" placeholder="Nombre del producto">
                    </td>
                    <td><input type="number" step="0.01" min="0" class="integraCantidad" value="${p.cantidad_fisica ?? p.cantidad ?? ''}"></td>
                    <td>
                        <select class="integraCategoria">
                ...
                `;
                if (p.categoria) row.querySelector('.integraCategoria').value = ['BOTELLAS Y GALONERAS','CAJAS','TAPAS Y ACCESORIOS','PREFORMAS Y SERVICIOS','ETIQUETAS','General'].includes(p.categoria) ? p.categoria : 'General';
                if (p.unidad_medida) row.querySelector('.integraUnidad').value = p.unidad_medida;
                row.dataset.nombreOriginal = p.nombre || '';
                tbody.appendChild(row);
            });
            document.getElementById('modalIntegrarManualmente').style.display = 'flex';
        }

        function cerrarModalIntegrarManualmente() {
            document.getElementById('modalIntegrarManualmente').style.display = 'none';
            ingresoIntegracionActual = null;
        }

        let guardIntegracionManual = false;
        async function confirmarIntegracionManual() {
            if (guardIntegracionManual) return;
            const filas = document.querySelectorAll('#tablaIntegracionManual tr');
            const itemsAjustados = [];
            filas.forEach(f => {
                const inputNombre = f.querySelector('.integraNombre');
                const inputCantidad = f.querySelector('.integraCantidad');
                if (inputNombre && inputNombre.value.trim()) {
                    itemsAjustados.push({
                        nombre: f.dataset.nombreOriginal || inputNombre.value.trim(),
                        nombre_ajustado: inputNombre.value.trim(),
                        cantidad_fisica: inputCantidad ? parseFloat(inputCantidad.value) || 0 : 0,
                        categoria: (f.querySelector('.integraCategoria') || {}).value || 'General',
                        unidad_medida: (f.querySelector('.integraUnidad') || {}).value || 'UNIDADES'
                    });
                }
            });
            if (itemsAjustados.length === 0) { alert('❌ Agrega al menos un producto con nombre válido.'); return; }
            if (!confirm(`Se integrarán ${itemsAjustados.length} ítem(s) manualmente al inventario. ¿Continuar?`)) return;
            guardIntegracionManual = true;
            try {
                const res = await fetch('/api/almacen/conformidad', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        ingreso_id: ingresoIntegracionActual,
                        usuario_almacen: localStorage.getItem('usuario_actual') || 'almacen1',
                ...
                alert(data.mensaje || 'Respuesta del servidor.');
                if (res.ok && data.success) {
                    cerrarModalIntegrarManualmente();
                    cargarPendientes();
                    cargarInventarioAlmacen();
                    cargarRegistroIngresos();
                } else {
                ...
                guardIntegracionManual = false;
            }
        }

        // 🗑️ ANULACIÓN DE PENDIENTE
        let ingresoAnularActual = null;
        function abrirModalAnularPendiente(id) {
            ingresoAnularActual = id;
            const guia = document.getElementById('anularGuiaInfo');
            if (guia) guia.textContent = ' #' + id;
            document.getElementById('anularObservacion').value = '';
            document.getElementById('modalAnularPendiente').style.display = 'flex';
        }

        function cerrarModalAnularPendiente() {
            document.getElementById('modalAnularPendiente').style.display = 'none';
            ingresoAnularActual = null;
        }

        let guardAnularPendiente = false;
        async function confirmarAnularPendiente() {
            if (guardAnularPendiente) return;
            const observacion = document.getElementById('anularObservacion').value.trim();
            if (!observacion) { alert('❌ Indica el motivo de la anulación.'); return; }
            if (!ingresoAnularActual) return;
            guardAnularPendiente = true;
            try {
                const res = await fetch('/api/almacen/pendientes/anular', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        ingreso_id: ingresoAnularActual,
                        usuario_almacen: localStorage.getItem('usuario_actual') || 'almacen1',
                        observacion
                    })
                });
            ...
            Object.keys(grupos).sort().reverse().forEach(fecha => {
                ...
                contenedor.innerHTML += `
                        <div class="grupo-fecha">
                            ...
                            <div class="tabla-wrap tabla-card-movil">
                        <table>
                            <thead>
                                <tr>
                                    <th>FECHA</th>
